<template>
  <div class="w-100 d-f j-c-c">
    ...
    <div class="left">
      ...
      <div class="accident d-f j-c-c a-i-c">
        <a-checkbox  @change="onChange">航空延误险：￥30/份×1 最高赔付300元</a-checkbox>
      </div>
    ...
    const placeOrder=(): void=>{
     ...
     data.arr.push(data.obj)
      ruleForm.value!.validate().then(()=>{
        api.postAirorders({seat_xid: data.item1.seat_xid,users: data.obj,contactName: data.form.username,contactPhone: data.form.phone,invoice: false,air: data.item1.ota_id}).then((res: any)=>{
          console.log(res);
        }).catch((err)=>{
          console.log(err);
        })
