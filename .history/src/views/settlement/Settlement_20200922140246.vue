    ...
    const placeOrder=(): void=>{
     ...
     data.arr.push(data.obj)
      ruleForm.value!.validate().then(()=>{
        api.postAirorders({insurances:data.insurances,seat_xid: data.item1.seat_xid,users: data.obj,contactName: data.form.username,contactPhone: data.form.phone,invoice: false,air: data.item1.ota_id}).then((res: any)=>{
          console.log(res);
        }).catch((err)=>{
          console.log(err);
        })
