    ...
    const placeOrder=(): void=>{
     ...
     data.arr.push(data.obj)
      ruleForm.value!.validate().then(()=>{
        api.postAirorders({seat_xid:data.item.seat_xid,users: data.obj,contactName: data.form.username,contactPhone:data.form.phone,invoice: false,air:1}).then((res:any)=>{
          console.log(res);
        }).catch((err)=>{
          console.log(err);
        })
