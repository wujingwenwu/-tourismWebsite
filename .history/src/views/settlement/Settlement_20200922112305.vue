    ...
    const data: Data = reactive<Data>({
      ...
      rules: {
        username: [
          ...
          { min: 6, max: 8, message: "长度在 2 到 4 位", trigger: "blur" },
    ...
    const placeOrder=(): void=>{
     ...
     data.arr.push(data.obj)
      ruleForm.value!.validate().then(()=>{
        api.postAirorders({seatxid: data.item1.seat_xid,users: data.obj,contactName: data.form.username,contactPhone: data.form.phone,invoice: false,air: data.item1.ota_id}).then((res: any)=>{
          console.log(res);
        }).catch((err)=>{
          console.log(err);
        })
