  ...
  <div class="d-f w-100 j-c-c">
    <div class="screener">
      <div class="d-f w-100 m-t-10">
        ...
        <div class="m-l-20 d-f">
          <div>
            <a-select v-model:value="values" style="width: 120px" placeholder="起飞机场" ref="select">
              <a-select-option :value="item" v-for="(item,index) in airport" :key="index">{{item}}</a-select-option>
            </a-select>
          </div>
          <div class="m-l-10">
            <a-select v-model:value="values1" style="width: 120px" placeholder="起飞时间" ref="select">
          ...
          <div class="m-l-10">
            <a-select v-model:value="values3" style="width: 120px" placeholder="机型" ref="select">
      ...
      <div>
        ...
        >
          <div>
            <div class="dynamic d-f a-i-c j-c-s-a" @click="lowPrice(index)">
            ...
            <div class="recommend d-f a-i-c" v-if="num===index">
              <div class="m-l-20">低价推荐</div>
              <div>
                <div v-for="(item1,index1) in item.seat_infos" :key="index1">
                  <div class="d-f paddings">
                    <div class="d-f">
                      <div class="green">{{item1.group_name}}</div>
                      <div class="show1 m-l-10"></div>
                      <div>{{item1.supplierName}}</div>
                    </div>
                    <div class="oranges orange">￥{{item1.org_settle_price}}</div>
                    <div class="oranges">
                      <div class="button d-f j-c-c a-i-c" @click="selected(item1,item)">选定</div>
                      <div class="m-l-10">剩余: {{item1.discount}}</div>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div class="m-l-20">
      <div class="right">
        ...
        <div class="telephone d-f j-c-c a-i-c">免费客服电话：4006345678转2</div>
      </div>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
        ...
        .then((res: any) => {
          ...
          res.flights.map((item: any) => {
            ...
            if (c < 0) {
              const d = c % (3600 * 1000);
              item.timeser = (b - a) / 1000 / 60 / 60;
              item.a = parseInt(item.timeser) + 23;
              item.b = Math.floor(d / 60 / 1000) + 60;
            }
          });
    ...
    const lowPrice = (index: number) => {
      console.log(index);
      data.num = index;
    };
    const selected = (item1: any, item: any) => {
      router.push({
        path: "/settlement",
        query: { item1: JSON.stringify(item1), item: JSON.stringify(item) },
      });
    };
    const handleChange = (): void => {
      console.log(data.values2);
      const ass = data.flights.filter((item) => {
        console.log(item.airline_name === item.airline_name);
        return data.value2 === item.airline_name;
      });
      console.log(ass);
    };
    return {
