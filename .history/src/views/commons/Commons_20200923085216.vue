<template>
  <div class="comment d-f f-d-c a-i-c">
    <div class="top d-f j-c-s-a">
      ...
      <div class="font login-hover hvr-underline-from-center" @click="clickLogin" v-if="!users">登录/注册</div>
    <div v-else class="d-f boxs">
    <a-popover placement="bottom">
        <template v-slot:content>
          <p @click="clickmine" class="mines">个人中心</p>
          <p @click="clickOut" class="mines">退出</p>
        </template>
   <div class="d-f">
        <div>
     <div class="boxs1">
     </div>
     <img :src="`http://157.122.54.189:9095${users.defaultAvatar}`" alt="" class="logo">
    </div>
    
    <div class="nickname m-l-10">{{users.nickname}}</div>
...
<script lang="ts">
import {defineComponent, reactive, toRefs,ComputedRef,computed,onMounted} from 'vue';
...
const clickmine=(): void=>{
 message.warning('抱歉，此功能暂未开发')
};
 onMounted(()=>{
   data.index=JSON.parse(localStorage.getItem("index")!)
 })  
return {
...toRefs(data),
clickHome,
